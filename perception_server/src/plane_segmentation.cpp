...
segmentPlaneAndObjects(
    ...
    pcl::ModelCoefficients::Ptr cached_coefficients // <--- MY_MOD: Cached plane coefficients
  ) {
  ...
  // FASE 1: DECISIONE (CACHE vs CALCOLO)
  // =========================================================
  if (cached_coefficients != nullptr) {
      // --- PERCORSO VELOCE (CACHE) ---
      LOG_INFO("Using CACHED plane coefficients. Skipping Normal Estimation and RANSAC.");
      *best_plane_model = *cached_coefficients;

      // NOTA: Qui saltiamo ne.compute(), clustering, RANSAC, ecc.
      // Andiamo dritti alla FASE 2.
  } 
  else {
