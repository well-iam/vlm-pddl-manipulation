...
fitBoxToCluster(
  ...
  // MY_MOD: Step 3: Calculate the midpoint of inlier points on the dominant line
  double mid_along = (min_along + max_along) / 2.0;
  double mid_perp = (min_perp + max_perp) / 2.0;  // <--- MY_MOD

  // MY_MOD: Step 4: Calculate the center point
  ...
  // MY_MOD: Step 5: Determine z-Position of the Box
...
std::vector<moveit_msgs::msg::CollisionObject> segmentObjects(
    ...
    const std::string& target_shape,    // <--- MY_MOD: ADDED
  ...
  for (const auto& cluster : cloud_clusters) {
    ...
    for (int i = 0; i < num_iterations; ++i) {
      ...
      while (static_cast<int>(projected_cloud->points.size()) > inlier_threshold) {
        ...
        // Variabili per i risultati (inizializzate vuote)
        pcl::PointIndices::Ptr line_inliers(new pcl::PointIndices);
        pcl::ModelCoefficients::Ptr line_coefficients(new pcl::ModelCoefficients);
        pcl::PointIndices::Ptr filtered_line_inliers(new pcl::PointIndices);
        ...
        // --- RAMO BOX (LINEE) ---
        // Esegui SOLO se il target è "box" (o non specificato/misto)
        if (target_shape != "cylinder") {
            auto result = fitLineRANSAC(projected_cloud,
                                        ransac_distance_threshold,
                                        ransac_max_iterations,
                                        projection_map);
            line_inliers = std::get<0>(result);
            line_coefficients = std::get<1>(result);

            // Filtering Linee
            filtered_line_inliers = filterLineInliers(
              line_inliers,
              cluster,
              projection_map,
        ...
        // --- RAMO CILINDRO (CERCHI) ---
        // Esegui SOLO se il target è "cylinder" (o non specificato/misto)
        if (target_shape != "box") {
            auto result = fitCircleRANSAC(projected_cloud,
                                          ransac_distance_threshold,
                                          ransac_max_iterations,
                                          hough_max_radius,
                                          projection_map);
            circle_inliers = std::get<0>(result);
            circle_coefficients = std::get<1>(result);

            // Filtering Cerchi
            filtered_circle_inliers = filterCircleInliers(
              circle_inliers,
              cluster,
              circle_coefficients,
              projection_map,
        ...
        if (target_shape != "box" && static_cast<int>(filtered_circle_inliers->indices.size()) > inlier_threshold) {
        ...
        // Validate Line Model
        if (target_shape != "cylinder" && static_cast<int>(filtered_line_inliers->indices.size()) > inlier_threshold) {
        ...
        // MY_MOD: Racing (Optional)
        // If we have not forced a type, we keep the original comparing logic.
        // If we have forced "box", valid_models will only contain lines, therefore this block won't cause problems.
        // Additional validation step: Compare inlier counts between circle and line models
        if (!valid_models.empty() && target_shape != "box" && target_shape != "cylinder") {
    ...
    if (top_model_type == "line") {
      ...
      // v---------------- MY_MOD: Return a box through PCA.
      LOG_INFO("  No valid model found via RANSAC. Attempting Generic Box Fallback (PCA)...");
      
      // FALLBACK STRATEGY:
      // Se RANSAC fallisce (la forma è strana, come Mario), usiamo la PCA sul cluster proiettato
      // per trovare l'asse principale e orientare la scatola.
      
      // 1. Compute the 2D Centroid
      Eigen::Vector2f centroid(0.0f, 0.0f);
      for (const auto& pt : projected_cloud->points) {
        centroid[0] += pt.x;
        centroid[1] += pt.y;
      }
      centroid /= static_cast<float>(projected_cloud->points.size());

      // 2. Compute co-variance matrix
      Eigen::Matrix2f covariance = Eigen::Matrix2f::Zero();
      for (const auto& pt : projected_cloud->points) {
        Eigen::Vector2f diff;
        diff[0] = pt.x - centroid[0];
        diff[1] = pt.y - centroid[1];
        covariance += diff * diff.transpose();
      }
      covariance /= static_cast<float>(projected_cloud->points.size());

      // 3. Eigenvalues and Eigenvectors to find the principal orientation
      Eigen::SelfAdjointEigenSolver<Eigen::Matrix2f> solver(covariance);
      Eigen::Vector2f eigenval = solver.eigenvalues();
      Eigen::Matrix2f eigenvec = solver.eigenvectors();

      // The eigenvector corresponding to the biggest eigenvalue is the principal axis
      // Eigen puts the eigenvalues in increasing order, therefore the last columnt is the principal axis
      Eigen::Vector2f principal_axis = eigenvec.col(1);

      // 4. Compute Theta and Rho to re-utilize the default function fitBoxToCluster
      // Theta is the orthogonal angle to the principal axis (fitBoxToCluster uses the normal to the line)
      double theta = std::atan2(principal_axis.y(), principal_axis.x()) + M_PI_2; 
      
      // Theta normalization
      if (theta < 0) theta += M_PI;
      
      // Rho is the distance from the origin to the line, passing through the centroid
      // Normal line equation: x*cos(theta) + y*sin(theta) = rho
      double rho = centroid[0] * std::cos(theta) + centroid[1] * std::sin(theta);

      // We use the default fitBoxToCluster to compute the dimensions
      auto [primitive, box_pose] = fitBoxToCluster(cluster, rho, theta);

      // Collision Object Creation
      moveit_msgs::msg::CollisionObject collision_object;
      collision_object.header.frame_id = frame_id;
      collision_object.id = "box_" + std::to_string(box_count++); // Different ID to distinguish
      ...
      LOG_INFO("Added FALLBACK box object based on PCA.");
    }
    LOG_INFO("");
