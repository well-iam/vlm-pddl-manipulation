/**
 * @file get_scene_objects_server.cpp
 ...
 *
 * Service:
 *     get_scene_objects (perception_server/srv/GetSceneObjects):
...
#include "perception_server/cluster_extraction.h"
#include "perception_server/normals_curvature_and_rsd_estimation.h"
#include "perception_server/object_segmentation.h"
#include "perception_server/plane_segmentation.h"
#include <perception_server/srv/get_scene_objects.hpp>  // snake_case generato da ROS
...
// MY_MOD: added voxel filter
#include <pcl/filters/voxel_grid.h>

class GetSceneObjectsServer : public rclcpp::Node {
 public:
  GetSceneObjectsServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
      : Node("get_scene_objects_server", options) {
    declareParameters();
    createSubscribers();
    createService();
 ...
 private:
  ...
  // MY_MOD: ADDED VOXEL FILTER PARAMETER
  double voxel_leaf_size;

  // MY_MOD: Private variables to add a cached table
  pcl::ModelCoefficients::Ptr cached_plane_coefficients_ = nullptr;
  bool is_plane_cached_ = false;
  ...
  // Service
  rclcpp::Service<perception_server::srv::GetSceneObjects>::SharedPtr service;
  ...
  void declareParameters() {
    ...
    // MY_MOD: Declare parameter for Voxel Filter
    declare_parameter("voxel_leaf_size", 0.004, "Leaf size for Voxel Grid downsampling (meters)");
    ...
    // MY_MOD: Get parameter for Voxel Grid
  ...
  void createSubscribers() {
    // --- MY_MOD: FIX CRITICO PER STABILITÀ RETE ---
    // We use a modified QoS profile "SensorData" for being Best Effort.
    // This prevents the node to ask TCP/DDS retransmissions if the switch loses packages,
    // avoiding network clogging and triggering robot Reflex.
    
    rclcpp::QoS qos_profile = rclcpp::SensorDataQoS();
    qos_profile.keep_last(1);       // Keep only the last frame
    qos_profile.best_effort();      // If you loose a package, don't retransmit.
    qos_profile.durability_volatile();
    
    // Create subscriber for point cloud data
    point_cloud_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      point_cloud_topic,
      qos_profile, //10,  // QoS depth
      std::bind(&GetSceneObjectsServer::pointCloudCallback, this, std::placeholders::_1)
    );
    ...
    rgb_image_sub = this->create_subscription<sensor_msgs::msg::Image>(
      rgb_image_topic,
      qos_profile, //10,  // QoS depth
      std::bind(&GetSceneObjectsServer::rgbImageCallback, this, std::placeholders::_1)
    );
  ...
  void createService() {
    ...
    service = this->create_service<perception_server::srv::GetSceneObjects>(
      "get_scene_objects",
     std::bind(&GetSceneObjectsServer::handleService, this, std::placeholders::_1, std::placeholders::_2),
      qos.get_rmw_qos_profile()
    );
    /*
    service = this->create_service<perception_server::srv::GetSceneObjects>(
      "get_scene_objects",
      [this](const std::shared_ptr<perception_server::srv::GetSceneObjects::Request> request,
              std::shared_ptr<perception_server::srv::GetSceneObjects::Response> response) {
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr convertToPCL(
    ...
    try {
        ...
        if(voxel_leaf_size > 0.0001) {
          /* MY_MOD: modify for speed: VOXEL GRID DOWNSAMPLING */
          pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_filtered(new pcl::PointCloud<pcl::PointXYZRGB>);
          pcl::VoxelGrid<pcl::PointXYZRGB> voxel_filter;
          voxel_filter.setInputCloud(pcl_cloud);

          // LEAF SIZE: "cube" dimension.
          // 0.005f = 5mm. Only one point remains per 5mm.
          // Increase to 0.008f o 0.01f (1cm) if still slow.
          voxel_filter.setLeafSize((float)voxel_leaf_size, (float)voxel_leaf_size, (float)voxel_leaf_size); 
          voxel_filter.filter(*cloud_filtered);

          RCLCPP_INFO(this->get_logger(), "Downsampling: %zu -> %zu points. Leaf size: %f", 
                      pcl_cloud->size(), cloud_filtered->size(), (float)voxel_leaf_size);

          return cloud_filtered;
        }        
        RCLCPP_INFO(this->get_logger(), "Selected voxel_leaf_size is too small. Skipping voxel filter...");
  ...
  std::string identifyTargetObject(
    ...
    for (const auto& object : objects) {
      ...
      // MY_MOD: Added "any" to accept the closest shape between "box" e "cylinder"
      // Compare shape types
   ...
   **********************************************************************************/
  void handleService(
      const std::shared_ptr<perception_server::srv::GetSceneObjects::Request> request,
      std::shared_ptr<perception_server::srv::GetSceneObjects::Response> response) {
    ...
    // MY_MOD: Added the possibility for searching everything: "any"
    const std::vector<std::string> valid_shapes = {"cylinder", "box", "any"};
     ...
     **********************************************************************************/
    // MY_MOD:
    pcl::ModelCoefficients::Ptr coeffs_to_use = nullptr;
    if (!request->recompute_support_surface && is_plane_cached_) {
        coeffs_to_use = cached_plane_coefficients_;
    }
    ...
    // MY_MOD: Update the cache if we have re-computed
    if (request->recompute_support_surface || !is_plane_cached_) {
        cached_plane_coefficients_ = plane_coefficients;
        is_plane_cached_ = true;
    }
    ...
    std::vector<moveit_msgs::msg::CollisionObject> segmented_objects = segmentObjects(
      ...
      num_iterations,
      target_frame,
      request->target_shape,  // <--- MY_MOD: ADDED
      inlier_threshold,
      hough_radius_bins,
      hough_center_bins,
...
int main(int argc, char** argv) {
  try {
    ...
    auto node = std::make_shared<GetSceneObjectsServer>(options)
    ;
    ...
    RCLCPP_ERROR(rclcpp::get_logger("get_scene_objects_server"), "Caught exception: %s", e.what());
    return 1;
  } catch (...) {
    RCLCPP_ERROR(rclcpp::get_logger("get_scene_objects_server"), "Caught unknown exception");
    return 1;
  }
